...
const REFRESH_TTL_SEC = parseTtlToSeconds(
  env.JWT_REFRESH_TTL,
  60 * 60 * 24 * 30,
); // 30d por defecto
...
export class AuthService {
  ...
  async refresh(
    ...
    if (!session) {
      logger.warn(
        ...
        "Refresh token not found (possible reuse)",
      );
  ...
  async resetPassword(token: string, newPassword: string): Promise<void> {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Si el usuario ya no existe o está inactivo, tratamos como inválido
      if (!user || !user.activo) {
        throw new BadRequestError("Invalid or expired token");
      }

      // Evita “reset a la misma password”
      if (user.passwordHash) {
      ...
      // Invalida otros tokens activos del usuario
    ...
    // Revoca sesiones del usuario ya resuelto (sin query extra)
    await this.logoutAll(resolvedUserId);
