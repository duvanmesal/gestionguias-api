...
import { sessionListSelect, usuarioForLoginSelect, usuarioPublicSelect } from "./auth.select"
...
export class AuthRepository {
  ...
  async getPasswordResetCandidate(tokenHash: string, now: Date): Promise<PasswordResetCandidate | null> {
  ...
  async applyPasswordReset(args: {
    ...
    return prisma.$transaction(async (tx) => {
      const used = await tx.passwordResetToken.updateMany({
        where: { id: args.tokenId, tokenHash: args.tokenHash, usedAt: null, expiresAt: { gt: args.now } },
      ...
      await tx.passwordResetToken.updateMany({
        where: { userId: args.userId, usedAt: null, expiresAt: { gt: args.now } },
        data: { usedAt: args.now },
      })
  ...
  async getEmailVerifyCandidate(tokenHash: string, now: Date): Promise<EmailVerifyCandidate | null> {
  ...
  async applyEmailVerification(args: { tokenHash: string; tokenId: string; userId: string; now: Date }) {
    return prisma.$transaction(async (tx) => {
