...
import { generateEmailVerifyToken, hashEmailVerifyToken } from "../../../libs/crypto"
import { sendVerifyEmailEmail } from "../../../libs/email"
import { logger } from "../../../libs/logger"
import { logsService } from "../../../libs/logs/logs.service"
import { env } from "../../../config/env"

export async function verifyEmailRequestUsecase(req: Request, email: string): Promise<void> {
  const normalizedEmail = normalizeEmail(email)

  const user = await authRepository.findUserByEmailForEmailVerification(normalizedEmail)
