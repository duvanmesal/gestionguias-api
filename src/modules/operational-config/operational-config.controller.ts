...
import type {
  UpdateTurnoAssignmentModeBody,
  UpdateNoShowPenaltyDurationBody,
} from "./operational-config.schemas"
...
export class OperationalConfigController {
  ...
  async updateNoShowPenaltyDuration(req: Request, res: Response, next: NextFunction) {
    try {
      const actorUserId = req.user!.userId
      const body = req.body as UpdateNoShowPenaltyDurationBody
      const config = await operationalConfigService.updateNoShowPenaltyDurationHours(
        req,
        body.durationHours,
        actorUserId,
      )
